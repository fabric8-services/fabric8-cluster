...
func (c clusterService) Load(ctx context.Context, clusterID uuid.UUID) (*repository.Cluster, error) {
	...
	// hide all sensitive info from the returned cluster record
...
// LoadWithSensitiveInfo loads the cluster given its ID, including the sentitive info (token, etc)
// This method is allowed for the 'Auth' service account only
// returns a NotFoundError error if no cluster with the given ID exists, or an "error with stack" if something wrong happend
func (c clusterService) LoadWithSensitiveInfo(ctx context.Context, clusterID uuid.UUID) (*repository.Cluster, error) {
...
// This method is allowed for the `Auth` service account only
func (c clusterService) ListWithSensitiveInfo(ctx context.Context) ([]repository.Cluster, error) {
	if !auth.IsSpecificServiceAccount(ctx, auth.Auth) {
		log.Error(ctx, nil, "unauthorized access to cluster info")
		return []repository.Cluster{}, errors.NewUnauthorizedError("unauthorized access to cluster info")
	}
	clusters, err := c.Repositories().Clusters().List(ctx)
	if err != nil {
		return []repository.Cluster{}, err
